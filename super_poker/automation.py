"""Daily learning and guarded competition-cycle deployment for Hot Poker 3."""
...
class CycleSchedule:
    ...
    def from_env(cls) -> "CycleSchedule":
        raw = os.getenv("HOT_POKER_CYCLE_ANCHOR_UTC", "2026-07-16T12:00:00+00:00")
        anchor = datetime.fromisoformat(raw.replace("Z", "+00:00"))
        if anchor.tzinfo is None:
            anchor = anchor.replace(tzinfo=timezone.utc)
        return cls(
            anchor_utc=anchor.astimezone(timezone.utc),
            duration_hours=max(1, int(os.getenv("HOT_POKER_CYCLE_HOURS", "120"))),
            lead_hours=max(1, int(os.getenv("HOT_POKER_CYCLE_LEAD_HOURS", "6"))),
        )
...
def promote(candidate: Path, incumbent: Path, backup_dir: Path) -> Path | None:
    ...
    if incumbent.is_file():
        ...
        backup = backup_dir / f"hot_poker_3-{stamp}.joblib"
...
def distribute_approved(
    ...
    approved = approved_dir / f"hot_poker_3-{version}.joblib"
...
def run(
    ...
    if should_train:
        ...
        incumbent = artifacts / "hot_poker_3.joblib"
