...
# CPU thread control. Multi-threaded torch on this small-attention workload
# suffers severe contention (measured ~10x slowdown, ~32s vs ~3.4s single-thread),
# which risks the 180s validator timeout. Pin to 1 thread by default; override
# with POKER44_TORCH_THREADS if a box genuinely benefits from more.
try:
    torch.set_num_threads(int(os.getenv("POKER44_TORCH_THREADS", "1")))
        ...
        for epoch in range(int(self.n_epochs)):
            ...
            if self.verbose_metrics and len(val_chunks) > 0:
                try:
                    ...
                    pass  # diagnostics-only dependency; not required for training
