...
class MicroSessionDetectionSynapse(bt.Synapse):
    """Classify audited schema-v4.1 tournament micro-sessions (v3.0 tournaments).

    Validators populate ``items`` (four-decision strategic micro-sessions) and
    retain ground truth locally. Miners return one risk score per item.
    """
...
def _validate_micro_envelope(window_id: str, dataset_hash: str, query_id: str) -> None:
...
def validate_micro_session_request(synapse: MicroSessionDetectionSynapse) -> None:
    """Reject malformed or forbidden micro-session requests before inference."""

    _validate_micro_envelope(synapse.window_id, synapse.dataset_hash, synapse.query_id)
    ...
    for index, item in enumerate(synapse.items):
        ...
        if leaked:
            raise ValueError(
                f"items[{index}] contains forbidden fields: {sorted(leaked)}"
            )
